...
* LLM Attentions and algorithms that improve compute and memory efficieny, while preserving accuracy 
  (eg. MHA, GQA, MQA, vLLM etc)
...
Q: What algorithm is used for the auto-regressive decoding loop for Llama-v2 and GPTJ?

A: The Llama-v2 benchmark uses greedy search decoding, while the GPTJ benchmark uses the beam search algorithm described at a high level here: https://huggingface.co/blog/how-to-generate#beam-search. Specifically, we use a beam width of 4 and enable early termination.
